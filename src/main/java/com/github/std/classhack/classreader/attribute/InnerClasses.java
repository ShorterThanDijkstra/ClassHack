...
public final class InnerClasses implements AttributeInfo {
    ...
    private static ClassInfo parseClass(InputStream input, ClassFile.ClassFileBuilder metaData) throws IOException {
        ...
        String innerClassName = AttributeChecker.checkInnerClassInnerInfoIndex(innerClassIndex, metaData.constantPool);

        int outerClassIndex = readBytes2(input);
        String outerClassName = AttributeChecker.checkInnerClassOuterInfoIndex(outerClassIndex, metaData.constantPool);

        int classNameIndex = readBytes2(input);
        String className = AttributeChecker.checkInnerClassInnerName(classNameIndex, metaData.constantPool);
